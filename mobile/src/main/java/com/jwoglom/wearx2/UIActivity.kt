...
class UIActivity : ComponentActivity(), GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, MessageApi.MessageListener {
    ...
    override fun onMessageReceived(messageEvent: MessageEvent) {
        ...
        when (messageEvent.path) {
            "/to-phone/start-comm" -> {
                startCommService()
                dataStore.setupStage.value = dataStore.setupStage.value?.nextStage(SetupStage.WAITING_PUMPX2_INIT)
            }

            "/to-phone/comm-started" -> {
                dataStore.setupStage.value = dataStore.setupStage.value?.nextStage(SetupStage.PUMPX2_SEARCHING_FOR_PUMP)
            }
            ...
            "/to-phone/set-pairing-code" -> {
                ...
                if (dataStore.setupStage.value == SetupStage.PUMPX2_WAITING_FOR_PAIRING_CODE) {
            ...
            "/from-pump/pump-discovered" -> {
                dataStore.setupStage.value = dataStore.setupStage.value?.nextStage(SetupStage.PUMPX2_PUMP_DISCOVERED)
                dataStore.setupDeviceName.value = String(messageEvent.data)
            }

            "/from-pump/initial-pump-connection" -> {
                dataStore.setupStage.value = dataStore.setupStage.value?.nextStage(SetupStage.PUMPX2_INITIAL_PUMP_CONNECTION)
                dataStore.setupDeviceName.value = String(messageEvent.data)
            }

            "/from-pump/pump-model" -> {
                dataStore.setupStage.value = dataStore.setupStage.value?.nextStage(SetupStage.PUMPX2_PUMP_MODEL)
                dataStore.setupDeviceModel.value = String(messageEvent.data)
            }

            "/from-pump/missing-pairing-code" -> {
                dataStore.setupStage.value = dataStore.setupStage.value?.nextStage(SetupStage.PUMPX2_WAITING_FOR_PAIRING_CODE)
            }

            "/from-pump/invalid-pairing-code" -> {
                dataStore.setupStage.value = dataStore.setupStage.value?.nextStage(SetupStage.PUMPX2_INVALID_PAIRING_CODE)
            }

            "/from-pump/pump-connected" -> {
                dataStore.setupStage.value = dataStore.setupStage.value?.nextStage(SetupStage.PUMPX2_PUMP_CONNECTED)
                dataStore.setupDeviceName.value = String(messageEvent.data)
            }

            "/from-pump/pump-disconnected" -> {
                dataStore.setupStage.value = dataStore.setupStage.value?.nextStage(SetupStage.PUMPX2_PUMP_DISCONNECTED)
                dataStore.setupDeviceModel.value = String(messageEvent.data)
            }
        }
    }
